...
public partial class App : Application
{
    ...
    public static void ShowMessageWithUnexpectedError(bool exitProgram = true)
    {
        ...
        {
            logFile = _fileSystem.Path.Combine(_fileSystem.Path.GetDirectoryName(Assembly.GetEntryAssembly().Location), logFile);
    ...
    private async void Application_Startup(object sender, StartupEventArgs e)
    {
        ...
        bool anotherInstanceIsRunning = IsAnotherInstanceRunning();
        ...
        RecentFilesAppSettingsSerializer recentFilesSerializer = new(_appSettings);
        ...
        else
        {
            ...
            if (StartupArguments is not ExportArgs)
            {
                Welcome w = new()
                {
                    DataContext = mainVM.WelcomeViewModel
                };
                _ = w.ShowDialog();
            }
    ...
    private static bool IsAnotherInstanceRunning()
    {
        ...
        string currentFileLocation = currentProcess.MainModule.FileName;

        System.Collections.Generic.IEnumerable<Process> runningProcesses = from process in Process.GetProcessesByName(currentProcess.ProcessName)
