...
namespace AnnoDesigner.Helper
{
    ...
    {
        /// <summary>
        /// Searches input 2D boolean array for clusters of adjacent cells.
        ...
        /// <returns>Lazily evaluated integer rectangles of grouped cells.</returns>
        public IEnumerable<Rect> GroupAdjacentCells(bool[][] cells, bool returnSingleCells = false)
        {
            while (true)
            {
                ...
                if (largest.Width * largest.Height > 1)
                {
                    ...
                    for (var x = 0; x < largest.Width; x++)
                        for (var y = 0; y < largest.Height; y++)
                            cells[(int)largest.X + x][(int)largest.Y + y] = false;
                }
        ...
        private Rect FindLargestGroup(bool[][] cells)
        {
            ...
            {
                for (var y = 0; y < cells[x].Length; y++)
                    column[y] = cells[x][y] ? column[y] + 1 : 0;
