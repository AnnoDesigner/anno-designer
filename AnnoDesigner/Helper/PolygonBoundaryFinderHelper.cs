...
public static class PolygonBoundaryFinderHelper
{
    ...
    private static ((int x, int y), Direction) FindStart(bool[][] insidePoints, Direction[][] boundarySides)
    {
        ...
        {
            for (int x = 0; x < boundarySides.Length; x++)
            {
                for (int y = 0; y < boundarySides[x].Length; y++)
                {
        ...
        else
        {
            ...
            {
                for (int y = 0; y < insidePoints[x].Length; y++)
                {
                    if (insidePoints[x][y])
                    {
    ...
    public static IReadOnlyList<(int x, int y)> GetBoundaryPoints(bool[][] insidePoints, Direction[][] boundarySides = null)
    {
        ...
        int maxX = insidePoints.Length;
        int maxY = insidePoints[0].Length;
        ((int x, int y) startPoint, Direction startDirection) = FindStart(insidePoints, boundarySides);
        (int x, int y) point = startPoint;
        ...
        do
        {
            (int forwardX, int forwardY) = GetForwardCell(point, direction);
            (int rightX, int rightY) = GetRightCell(point, direction);
            ...
            else // turn left
            {
                if (boundarySides != null)
                {
                    (int leftX, int leftY) = GetLeftCell(point, direction);
    ...
    {
        Direction[][] boundarySides = GetBoundarySides(insidePoints);
