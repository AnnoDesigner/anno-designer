...
/// </remarks>
public class UpdateHelper(string basePathToUse,
    ...
    ILocalizationHelper localizationHelperToUse) : IUpdateHelper
{
    ...
    {
        List<AvailableRelease> result = [];

        AllReleases ??= await GetAllAvailableReleases().ConfigureAwait(false);

        foreach (ReleaseType curReleaseType in Enum.GetValues<ReleaseType>())
        {
            AvailableRelease foundRelease = CheckForAvailableRelease(curReleaseType);
    ...
    public async Task<AvailableRelease> GetAvailableReleasesAsync(ReleaseType releaseType)
    {
        ...
        AvailableRelease foundRelease = CheckForAvailableRelease(releaseType);
    ...
    public async Task<string> DownloadReleaseAsync(AvailableRelease releaseToDownload)
    {
        try
        {
            ...
            Release release = AllReleases.FirstOrDefault(x => x.Id == releaseToDownload.Id);
            if (release == null)
            {
            ...
            string assetName = GetAssetNameForReleaseType(releaseToDownload.Type);
            ...
            ReleaseAsset foundAsset = release.Assets.FirstOrDefault(x => x.Name.StartsWith(assetName, StringComparison.OrdinalIgnoreCase));
            ...
            string pathToDownloadedFile = await DownloadFileAsync(foundAsset.BrowserDownloadUrl, Path.GetTempFileName()).ConfigureAwait(false);
            ...
            FileInfo tempFileInfo = new(pathToDownloadedFile);

            string pathToUpdatedPresetsFile = GetPathToUpdatedPresetsFile(releaseToDownload.Type);
    ...
    public Task<bool> AreUpdatedPresetsFilesPresentAsync()
    {
        return Task.Run(() =>
        {
            foreach (ReleaseType curReleaseType in Enum.GetValues<ReleaseType>())
            {
                string pathToUpdatedPresetsFile = GetPathToUpdatedPresetsFile(curReleaseType);
        ...
        try
        {
            await Task.Run(async () =>
            {
                foreach (ReleaseType curReleaseType in Enum.GetValues<ReleaseType>())
                {
                    ...
                    {
                        using (ZipArchive archive = ZipFile.OpenRead(pathToUpdatedPresetsFile))
                        {
                            foreach (ZipArchiveEntry curEntry in archive.Entries)
                            {
                                string destinationPath = Path.Combine(Path.GetDirectoryName(pathToUpdatedPresetsFile), curEntry.FullName);
                    ...
                    string originalPresetsFileName = Path.GetFileName(pathToUpdatedPresetsFile).Replace(CoreConstants.PrefixUpdatedPresetsFile, string.Empty);
                    string pathToOriginalPresetsFile = Path.Combine(Path.GetDirectoryName(pathToUpdatedPresetsFile), originalPresetsFileName);
    ...
    public async Task<(bool, Version)> IsNewAppVersionAvailableAsync()
    {
        if (appSettingsToUse.UpdateSupportsPrerelease)
        {
            ...
            AvailableRelease foundRelease = CheckForAvailableRelease(ReleaseType.AnnoDesigner);
            if (foundRelease is null)
            {
                return (false, default);
            }

            bool isNewVersionAvailable = foundRelease.Version > Constants.Version;
        ...
        else
        {
            ...
            bool isNewVersionAvailable = Version.TryParse(downloadedContent, out Version parsedVersion) && parsedVersion > Constants.Version;
    ...
    private async Task<IReadOnlyList<Release>> GetAllAvailableReleases()
    {
        try
        {
            ...
            IReadOnlyList<Release> releases = await ApiClient.Repository.Release.GetAll(GITHUB_USERNAME, GITHUB_PROJECTNAME).ConfigureAwait(false);
    ...
    private AvailableRelease CheckForAvailableRelease(ReleaseType releaseType)
    {
        ...
        string tagToCheck = GetTagForReleaseType(releaseType);
        ...
        bool supportPrerelease = appSettingsToUse.UpdateSupportsPrerelease;
        ...
        Release foundRelease = AllReleases.FirstOrDefault(x => !x.Draft && !x.Prerelease && x.TagName.StartsWith(tagToCheck, StringComparison.OrdinalIgnoreCase));
    ...
    private Version ParseVersionFromTag(Release release, string tagToCheck)
    {
        ...
        string versionString = release.TagName.Replace(tagToCheck, string.Empty).Trim();
    ...
    private string GetPathToUpdatedPresetsFile(ReleaseType releaseType)
    {
        ...
        {
            case ReleaseType.Presets:
                result = Path.Combine(basePathToUse, CoreConstants.PrefixUpdatedPresetsFile + GetAssetNameForReleaseType(releaseType));
                break;
            case ReleaseType.PresetsAndIcons:
                result = Path.Combine(basePathToUse, CoreConstants.PrefixUpdatedPresetsFile + GetAssetNameForReleaseType(releaseType));
                break;
    ...
    private async Task<string> DownloadFileAsync(string url, string pathToSavedFile)
    {
        try
        {
            ...
            Stream stream = await LocalHttpClient.GetStreamAsync(url).ConfigureAwait(false);
            using (FileStream fileStream = new(pathToSavedFile, System.IO.FileMode.Create))
            {
