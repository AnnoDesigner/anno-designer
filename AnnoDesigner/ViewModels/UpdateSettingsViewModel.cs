...
public class UpdateSettingsViewModel : Notify
{
    ...
    private async Task CheckForPresetsAsync(bool isAutomaticUpdateCheck)
    {
        ...
        AvailableRelease foundRelease = await _updateHelper.GetAvailableReleasesAsync(ReleaseType.Presets);
        if (foundRelease == null)
        {
            return;
        }

        bool isNewReleaseAvailable = foundRelease.Version > new Version(PresetsVersionValue);
    ...
    private async void ExecuteDownloadPresets(object param)
    {
        ...
        string newLocation = await _updateHelper.DownloadReleaseAsync(FoundPresetRelease);
